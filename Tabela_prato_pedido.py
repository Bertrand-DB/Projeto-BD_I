...
class Tabela_prato_pedido():
    ...
    def label_entry(self):
        ...
        self.col2_entry.place(relx=X_LABEL+LARGURA_LABEL, rely=ESPACAMENTO_LB*3 + ALTURA_LABEL*2, relwidth=0.05, relheight=ALTURA_LABEL)
    ...
    def preencher(self, event):
        ...
        for n in self.lista.selection():
            col1, col2, col3 = self.lista.item(n, 'values')
    ...
    def insert_bt(self):
        ...
        if not self.fsql.insert_pratos_pedidos(entrys): return
        self.atualiza_tabela()
        self.clear_entry()

    def update_bt(self):
        entrys = self.get_entry()
        if not self.fsql.update_pratos_pedidos(entrys): return
        self.atualiza_tabela()
        self.clear_entry()
    
    def delete_bt(self):
        entrys = self.get_entry()
        if not self.fsql.delete_pratos_pedidos(entrys): return
        self.atualiza_tabela()
        self.clear_entry()
    ...
    def select_bt(self):
        ...
        encontrado = self.fsql.select_pratos_pedidos(entrys)
