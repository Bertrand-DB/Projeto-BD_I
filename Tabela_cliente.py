...
class Tabela_cliente():
    ...
    def label_entry(self):
        ...
        self.col0_entry.place(relx=X_LABEL+LARGURA_LABEL, rely=ESPACAMENTO_LB*1 + ALTURA_LABEL*0, relwidth=0.5, relheight=ALTURA_LABEL)

        self.col1_entry = Entry(self.quadro_1, width=25, justify='left', font=("Arial", 12), highlightthickness=1, relief="solid")
        self.col1_entry.place(relx=X_LABEL+LARGURA_LABEL, rely=ESPACAMENTO_LB*2 + ALTURA_LABEL*1, relwidth=0.5, relheight=ALTURA_LABEL)

        self.col2_entry = Entry(self.quadro_1, width=25, justify='left', font=("Arial", 12), highlightthickness=1, relief="solid")
        self.col2_entry.place(relx=X_LABEL+LARGURA_LABEL, rely=ESPACAMENTO_LB*3 + ALTURA_LABEL*2, relwidth=0.3, relheight=ALTURA_LABEL)

        self.col3_entry = Entry(self.quadro_1, width=25, justify='left', font=("Arial", 12), highlightthickness=1, relief="solid")
        self.col3_entry.place(relx=X_LABEL+LARGURA_LABEL, rely=ESPACAMENTO_LB*4 + ALTURA_LABEL*3, relwidth=0.022, relheight=ALTURA_LABEL)
    ...
    def insert_bt(self):
        ...
        if not self.fsql.insert_clientes(entrys): return
        self.atualiza_tabela()
        self.clear_entry()

    def update_bt(self):
        entrys = self.get_entry()
        if not self.fsql.update_clientes(entrys): return
        self.atualiza_tabela()
        self.clear_entry()
    
    def delete_bt(self):
        entrys = self.get_entry()
        if not self.fsql.delete_clientes(entrys): return
        self.atualiza_tabela()
        self.clear_entry()
    ...
    def select_bt(self):
        ...
        encontrado = self.fsql.select_clientes(entrys)
