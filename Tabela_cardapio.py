...
class Tabela_cardapio():
    ...
    def label_entry(self):
        ...
        self.col2_entry.place(relx=X_LABEL+LARGURA_LABEL, rely=ESPACAMENTO_LB*3 + ALTURA_LABEL*2-0.06, relwidth=0.4, relheight=ALTURA_LABEL)

        self.col3_entry = Entry(self.quadro_1, width=25, justify='left', font=("Arial", 12), highlightthickness=1, relief="solid")
        self.col3_entry.place(relx=X_LABEL+LARGURA_LABEL, rely=ESPACAMENTO_LB*4 + ALTURA_LABEL*3-0.06, relwidth=0.6, relheight=ALTURA_LABEL)

        self.col4_entry = Entry(self.quadro_1, width=25, justify='left', font=("Arial", 12), highlightthickness=1, relief="solid")
        self.col4_entry.place(relx=X_LABEL+LARGURA_LABEL, rely=ESPACAMENTO_LB*5 + ALTURA_LABEL*4-0.06, relwidth=0.1, relheight=ALTURA_LABEL)

        self.col5_entry = Entry(self.quadro_1, width=25, justify='left', font=("Arial", 12), highlightthickness=1, relief="solid")
        self.col5_entry.place(relx=X_LABEL+LARGURA_LABEL, rely=ESPACAMENTO_LB*6 + ALTURA_LABEL*5-0.06, relwidth=0.075, relheight=ALTURA_LABEL)
    ...
    def insert_bt(self):
        ...
        if not self.fsql.insert_cardapio(entrys): return
        self.atualiza_tabela()
        self.clear_entry()
    ...
    def update_bt(self):
        ...
        entrys = self.get_entry()
        if not self.fsql.update_cardapio(entrys): return
        self.atualiza_tabela()
        self.clear_entry()
    
    def delete_bt(self):
        entrys = self.get_entry()
        if not self.fsql.delete_cardapio(entrys): return
        self.atualiza_tabela()
        self.clear_entry()
    ...
    def select_bt(self):
        ...
        encontrado = self.fsql.select_cardapio(entrys)
